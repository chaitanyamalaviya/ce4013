...
public class Server {
	...
	public static Server unmarshal(byte[] request) {
		...
		case "D":
			break;
		case "M":
			ob.monitorInterval = Integer.parseInt(true_request.substring(ob.length + 5, ob.length + 9));
			break;
		case "F":
			int destPathLen = Integer.parseInt(true_request.substring(ob.length + 5, ob.length + 9));
			ob.destPath = true_request.substring(ob.length + 9, ob.length + 9 + destPathLen);
			break;
